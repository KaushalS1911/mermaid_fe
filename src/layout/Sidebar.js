...
const Sidebar = ({open, mobileOpen, handleDrawerToggle, setSidebarKey, sidebarKey}) => {
    ...
    const menuItems = [
        {text: "Collapse menu", icon: <KeyboardArrowRightIcon/>, action: () => setCollapsed(!collapsed)},
        {text: "Templates", icon: <DataSaverOffIcon/>},
        {text: "Snippets", icon: <DashboardCustomizeIcon/>},
        {text: "Design", icon: <BrushIcon/>, action: (event) => setDesignAnchor(event.currentTarget)},
        {text: "Export", icon: <SystemUpdateAltIcon/>, action: (event) => setExportAnchor(event.currentTarget)},
    ];
